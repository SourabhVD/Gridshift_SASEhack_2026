...
Everything the reference backend can be told is read here, once, at import
time. `.env` (next to this package's parent) is loaded first so that running
`uvicorn app.main:app` from `backend/reference/` picks it up without any shell
exports; real environment variables always win over the file.
"""
